...
export const getAllInstagramOrders = async (req, res, next) => {
  try {
    ...
    if (startValid && endValid && startDate > endDate) {
      return res.status(200).json(new ApiResponse(200, "Instagram orders fetched successfully", {
        orders: [],
    ...
    const [orders, totalOrders] = await Promise.all([
      InstagramOrder.find(filter).sort({ [sortBy]: sortOrder }).skip(skip).limit(limit).lean(),
...
export const createInstagramOrder = async (req, res, next) => {
  try {
    ...
    const unavailableNames = productIds
      .filter((id) => ["out_of_stock", "discontinued"].includes(productMap.get(id).productStatus))
      .map((id) => productMap.get(id).productName);
    if (unavailableNames.length > 0) throw new ApiError(400, `Unavailable products: ${unavailableNames.join(", ")}`);
    ...
    const amount = builtItems.reduce(
      (sum, item) => sum + item.productSnapshot.priceAtPurchase * item.productSnapshot.quantity, 0
    );
    ...
    return res.status(201).json(new ApiResponse(201, "Instagram order created successfully", order));
  } catch (err) {
    next(err);
  }
};
...
export const streamInstagramOrders = (req, res) => {
  res.writeHead(200, {
  ...
  console.log(`[SSE:Instagram] Client connected (${req.admin?.email ?? "unknown"})`);

  req.on("close", () => {
    clearInterval(heartbeat);
    orderEventEmitter.off("new_instagram_order", sendOrder);
    console.log(`[SSE:Instagram] Client disconnected (${req.admin?.email ?? "unknown"})`);
  });
};
