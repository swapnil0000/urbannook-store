...
async function verifySchema() {
  try {
    ...
    console.log("Database URI:", process.env.MONGODB_URI?.replace(/\/\/([^:]+):([^@]+)@/, '//$1:****@'));
    ...
    console.log("\n🗄️  PRODUCTION MongoDB Collection Info:");
    console.log("=" .repeat(80));
    ...
    if (productCollection) {
      ...
      if (sampleDoc) {
        ...
        if ('colorOptions' in sampleDoc) {
          console.log("\n   ✅ colorOptions exists in PRODUCTION database documents");
          console.log(`   Value: ${JSON.stringify(sampleDoc.colorOptions)}`);
        } else {
          console.log("\n   ❌ colorOptions does NOT exist in PRODUCTION database documents");
        }
      } else {
        console.log("\n   ⚠️  No products found in PRODUCTION database");
      }
    }
