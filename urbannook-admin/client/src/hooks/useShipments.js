  ...
  switch (action.type) {
    ...
    case "OPEN_TRACK_DRAWER":
      return {
        ...state,
        actionMenu:  { open: false, shipmentId: null },
        trackDrawer: { open: true, shipment: action.payload, trackData: null, loading: true, error: null },
      };
    case "SET_TRACK_DATA":
      return { ...state, trackDrawer: { ...state.trackDrawer, loading: false, trackData: action.payload } };
    case "SET_TRACK_ERROR":
      return { ...state, trackDrawer: { ...state.trackDrawer, loading: false, error: action.payload } };
    case "CLOSE_TRACK_DRAWER":
      return { ...state, trackDrawer: { open: false, shipment: null, trackData: null, loading: false, error: null } };
    ...
    case "OPEN_LABEL_MODAL":
      return {
        ...state,
        actionMenu:  { open: false, shipmentId: null },
        labelModal:  { open: true, shipment: action.payload, label: null, loading: true, error: null },
      };
    case "SET_LABEL_DATA":
      return { ...state, labelModal: { ...state.labelModal, loading: false, label: action.payload } };
    case "SET_LABEL_ERROR":
      return { ...state, labelModal: { ...state.labelModal, loading: false, error: action.payload } };
    case "CLOSE_LABEL_MODAL":
      return { ...state, labelModal: { open: false, shipment: null, label: null, loading: false, error: null } };
    ...
    case "SET_CANCEL_LOADING":
      return { ...state, cancelDialog: { ...state.cancelDialog, loading: true, error: null } };
    case "SET_CANCEL_ERROR":
      return { ...state, cancelDialog: { ...state.cancelDialog, loading: false, error: action.payload } };
    case "CLOSE_CANCEL_DIALOG":
      return { ...state, cancelDialog: { open: false, shipment: null, loading: false, error: null } };
...
export function useShipments() {
  ...
  const refetch = useCallback(() => fetchShipments(state.activeTab, state.currentPage), [
    state.activeTab, state.currentPage, fetchShipments,
  ]);

  // ── Action menu ─────────────────────────────────────────────────────────────
  const openActionMenu  = useCallback((shipmentId) => dispatch({ type: "OPEN_ACTION_MENU",  payload: shipmentId }), []);
  const closeActionMenu = useCallback(() => dispatch({ type: "CLOSE_ACTION_MENU" }), []);

  // ── Assign courier ──────────────────────────────────────────────────────────
  const openAssignDrawer  = useCallback((shipment) => dispatch({ type: "OPEN_ASSIGN_DRAWER",  payload: shipment }), []);
  const closeAssignDrawer = useCallback(() => dispatch({ type: "CLOSE_ASSIGN_DRAWER" }), []);
  ...
  const confirmAssign = useCallback(async (shipmentId, courierId) => {
    const res = await apiClient.post(`/admin/shipmozo/shipments/${shipmentId}/assign`, { courierId });
  ...
  const openTrackDrawer  = useCallback(async (shipment) => {
    ...
    try {
      ...
      if (res.data.data?.current_status) {
        dispatch({ type: "UPDATE_SHIPMENT", payload: { _id: shipment._id, shipmentStatus: res.data.data.current_status } });
      }
      dispatch({ type: "SET_TRACK_DATA", payload: res.data.data });
    } catch (err) {
      dispatch({ type: "SET_TRACK_ERROR", payload: err.response?.data?.message || "Failed to fetch tracking data." });
    }
  }, []);
  ...
  const openLabelModal  = useCallback(async (shipment) => {
    ...
    try {
      ...
      dispatch({ type: "SET_LABEL_ERROR", payload: err.response?.data?.message || "Failed to fetch shipping label." });
    }
  }, []);
  const closeLabelModal = useCallback(() => dispatch({ type: "CLOSE_LABEL_MODAL" }), []);

  // ── Cancel ──────────────────────────────────────────────────────────────────
  const openCancelDialog  = useCallback((shipment) => dispatch({ type: "OPEN_CANCEL_DIALOG", payload: shipment }), []);
  const closeCancelDialog = useCallback(() => dispatch({ type: "CLOSE_CANCEL_DIALOG" }), []);

  const confirmCancel = useCallback(async (shipmentId) => {
    dispatch({ type: "SET_CANCEL_LOADING" });
    try {
      const res = await apiClient.post(`/admin/shipmozo/shipments/${shipmentId}/cancel`);
      dispatch({ type: "UPDATE_SHIPMENT", payload: res.data.data });
